...
function SelectWithCreate({
  ...
  useEffect(() => {
    getSchema(templateId).then((res) => {
      ...
      if (formData[propName]) {
        ...
        if (pattern.length > 0) {
          Promise.all(formData?.[fragmentId]?.[propName].filter((el) => el.updateType !== 'delete').map((el) => parsePatern(el, pattern))).then(
            (listParsed) => {
              setlist(listParsed);
            }
          );
  ...
  const handleAddToList = () => {
    ...
    if (checkForm) {
      ...
      if (index !== null) {
        ...
        const parsedPattern = parsePattern(subData, registerFile.to_string);
  ...
  const handleSave = () => {
    ...
    setlist([...list, parsePattern(subData, registerFile.to_string)]);
    handleClose();
    setSubData(null);
  };
